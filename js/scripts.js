		...
		if((width < custom_width)) {
			...
			$(window).on('scroll', function(){
				...
				$('.top-menu ul li a').each(function () {
					...
					if (refElement.offset().top - 150<= scrollPos) {
...
$('.copy-ref').click(function() {
	...
	alert("Reference have been copied to the clipboard")
	navigator.clipboard.writeText($(this).parents('.references')[0].innerHTML.replace("<br>", "").replace("Copy", "").trim().replace(/\t+/g, '\t'))
	console.log($(this).parents('.references')[0].innerHTML.replace("<br>", "").replace("Copy", "").trim().replace(/\t+/g, '\t'));
	console.log($(this).closest('div .references').children().text());
	// alert("Button clicked, text"+this.innerHTML);
});
// function copyBibtext(){
// 	alert(this)
// 	alert("Button clicked, text"+this.querySelector('.references').innerHTML);
// }